...
def send_all_csv_students_to_s3():
    s3_client.put_object(Bucket=bucket_name, Body=concat_all3files().to_csv(), Key='Allcsv/all3concat.csv')
...
def upload_alltxt_as_csv(s3_client, bucket_name, key='Allcsv/SpartaDaytxt.csv'):
...
def get_all_files_folder(s3_client, bucket_name, folder):
    ...
    response = paginator.paginate(Bucket=bucket_name, Prefix=folder, PaginationConfig={"MaxItems": 4000})
    fij = []
...
def upload_applicants(filename):
